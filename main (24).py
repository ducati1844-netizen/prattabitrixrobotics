...
PIPELINE_STAGES = {}
# Кэш для защиты от дублирования событий
import threading as _threading
_processed_events = {}
_events_lock = _threading.Lock()

def is_duplicate_event(key, ttl_seconds=10):
    """Возвращает True если событие уже обрабатывалось в последние ttl_seconds секунд"""
    import time
    now = time.time()
    with _events_lock:
        if key in _processed_events:
            if now - _processed_events[key] < ttl_seconds:
                return True
        _processed_events[key] = now
        # Чистим старые события
        expired = [k for k, v in _processed_events.items() if now - v > ttl_seconds * 2]
        for k in expired:
...
def create_task(title, responsible_id, deal_id, description="", deadline_hours=24):
    # Railway работает в UTC, Битрикс24 в UTC+7 — добавляем 7 часов
    deadline = (datetime.now() + timedelta(hours=deadline_hours + 7)).strftime("%Y-%m-%dT%H:%M:%S")
    rop_id = USERS["rop"]
...
def on_stage_change(deal, old_stage_id, new_stage_id):
    try:
        ...
        event_key = f"stage_{did}_{new_stage_id}"
        if is_duplicate_event(event_key):
            print(f"[SKIP] Дубль события: {event_key}")
            return
        # Определяем воронку
        ...
        if cat_id != "0":
            for p in (b24("crm.dealcategory.list") or []):
                if str(p["ID"]) == cat_id:
                    pipeline = p["NAME"]
                    break
        print(f"[STAGE] {title}: {old_stage_id} -> '{new_stage}' ({new_stage_id}) [{pipeline}]")
        ...
        elif new_stage == "Receipt":
            create_task(f"Выдать Receipt клиенту — {title}", USERS["buh"], did,
                f"Клиент: {contact}\nОплата получена. Выдать Receipt и перевести сделку в Колеровку.", 2)
...
TASK_NEXT_STAGE = {
    ...
    "Запросить отзыв —":         "Закрыто успешно",
}

def ensure_stages_loaded():
    """Всегда загружает актуальные стадии из Битрикс24"""
    load_stages()
...
def on_task_complete(task):
    try:
        ...
        # НЕ вызываем on_stage_change здесь - Битрикс24 сам пришлёт ONCRMDEALUPDATE
...
# Стадии загружаются динамически при каждом запросе через ensure_stages_loaded()
