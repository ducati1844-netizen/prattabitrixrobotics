...
TASK_NEXT_STAGE = {
    "Связаться с клиентом":        ("Квалификация",           "Paint — Продажа краски"),
    "Квалификация клиента":        ("Согласование",            "Paint — Продажа краски"),
    "Согласовать детали":          ("Коммерческое предложение","Paint — Продажа краски"),
    "Подготовить КП":              ("Согласование и дожим",    "Paint — Продажа краски"),
    "Получить ответ по КП":        ("Счёт и договор",          "Paint — Продажа краски"),
    "Выставить Invoice":           ("Оплата",                  "Paint — Продажа краски"),
    "Проконтролировать оплату":    ("Колеровка",               "Paint — Продажа краски"),
    "Списать товар":               ("Колеровка",               "Paint — Продажа краски"),
    "Выполнить колеровку":         ("Отгрузка и логистика",    "Paint — Продажа краски"),
    "Организовать отгрузку":       ("Завершено",               "Paint — Продажа краски"),
    # Розница B2C
    "Провести квалификацию":       ("Согласование",            "Розница B2C"),
    "Подобрать материал":          ("Образцы",                 "Розница B2C"),
    "Подготовить образцы":         ("Коммерческое предложение","Розница B2C"),
    # Нанесение
    "Подтвердить дату старта":     ("Подготовка объекта",      "Нанесение"),
    "Проверить готовность":        ("Заказ материала",         "Нанесение"),
    "Заказ материала":             ("Колеровка",               "Нанесение"),
    "Выход бригады":               ("В работе",                "Нанесение"),
    "Подписать акт":               ("Закрыта успешно",          "Нанесение"),
    "Провести инспекцию":          ("Объект готов",             "Нанесение"),
    "Тестовая стена":              ("Тест согласован",          "Нанесение"),
    "Подготовка к выходу":         ("В работе",                 "Нанесение"),
    "Финальная приемка":           ("Приемка",                  "Нанесение"),
}

def on_task_complete(task_id):
    try:
        # Получаем данные задачи
        task = b24("tasks.task.get", {"taskId": task_id, "select": ["ID","TITLE","UF_CRM_TASK","STATUS"]})
        if not task:
            return
        task = task.get("task", task)
        title = task.get("TITLE", "")
        crm_links = task.get("UF_CRM_TASK", [])
        ...
        next_stage = None
        next_pipeline = None
        for keyword, (stage, pip) in TASK_NEXT_STAGE.items():
            if keyword.lower() in title.lower():
                # Проверяем что воронка совпадает или задача универсальная
                if pip == pipeline or pip == "Paint — Продажа краски":
                    next_stage = stage
        ...
        # Получаем ID следующей стадии
        stage_id = get_stage_id(pipeline, next_stage)
        if not stage_id:
            print(f"[TASK DONE] Стадия не найдена: {next_stage} в {pipeline}")
            return

        current_stage = deal.get("STAGE_ID", "")
        # Не двигаем если сделка уже дальше
        result = move_deal(deal_id, stage_id)
        print(f"[TASK DONE] Сделка {deal_id} -> {next_stage} ({stage_id})")
...
def webhook():
    ...
    if event == "ONCRMDEALADD":
        ...
        # Данные приходят в FIELDS_AFTER
        task_id = (data.get("data[FIELDS_AFTER][ID]") or 
                   data.get("data[FIELDS][ID]") or
                   data.get("data[ID]"))
        status = str(data.get("data[FIELDS_AFTER][STATUS]") or 
                     data.get("data[FIELDS][STATUS]") or "")
        print(f"[TASK UPDATE] id={task_id} status={status}")
        # STATUS=5 = завершена в Битрикс24
        if task_id and status == "5":
            threading.Thread(target=on_task_complete, args=(task_id,), daemon=True).start()
        else:
            print(f"[TASK UPDATE] Пропускаем: id={task_id} status={status}")
