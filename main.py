...
def check_task_status(task_id):
    ...
    try:
        result = b24("tasks.task.get", {"taskId": task_id, "select": ["ID","TITLE","STATUS","UF_CRM_TASK"]})
        if not result:
            return
        task = result.get("task", result)
        status = str(task.get("STATUS", ""))
        title = task.get("TITLE", "")
        print(f"[TASK CHECK] id={task_id} status={status} title={title[:50]}")
        # STATUS=5 = завершена
        if status == "5":
...
def on_task_complete(task):
    try:
        # task уже является объектом задачи
        if isinstance(task, (str, int)):
            # на случай если передали ID
            result = b24("tasks.task.get", {"taskId": task, "select": ["ID","TITLE","UF_CRM_TASK","STATUS"]})
            if not result:
                return
            task = result.get("task", result)
        title = task.get("TITLE", "")
