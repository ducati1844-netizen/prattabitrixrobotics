...
TASK_NEXT_STAGE = {
    ...
    "Выход бригады":               ("Выполнение работ",        "Нанесение"),
    "Подписать акт":               ("Завершено",               "Нанесение"),
}
...
def webhook():
    ...
    if event == "ONCRMDEALADD":
        ...
        task_id = data.get("data[FIELDS][ID]")
        status = str(data.get("data[FIELDS][STATUS]", ""))
        print(f"[TASK UPDATE] id={task_id} status={status} all_keys={[k for k in data.keys() if 'FIELDS' in k]}")
        ...
        if task_id and status == "5":
            ...
            print(f"[TASK UPDATE] Статус {status} - не завершена, пропускаем")
